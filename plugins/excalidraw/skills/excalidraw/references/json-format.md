...
| `frame`     | Grouping containers that clip children (preferred over dashed rectangles)   | N/A               |
...
### Option 1: Frame Elements (Preferred)

Frames are native container elements that clip and group children. Child elements reference the frame via `frameId`:

```json
{
  "id": "frame-ai-pipeline",
  "type": "frame",
  "x": 100,
  "y": 500,
  "width": 1000,
...
Child elements inside the frame set `frameId` to the frame's ID:

```json
{
  "id": "convert-service",
  "type": "rectangle",
  "frameId": "frame-ai-pipeline",
  // ... other properties
}
```

**Frame properties:**
- `name`: Displayed as a label above the frame (string or `null`)
- Children are clipped to frame boundaries
- Frames render as a subtle bordered region

### Option 2: Dashed Rectangles (Simpler)

For lightweight visual grouping without clipping:
...
|--------------------|-----------------------------------------|------------------------------|
| Frame (`frame`)    | VPCs, namespaces, deployment boundaries | Clips children, native label |
| Dashed rectangle   | Loose grouping, visual hints            | Simpler, no clipping         |
