...
For logical groupings, use either:
- **Frame elements** (`type: "frame"`) - native containers with clipping and built-in labels. Child elements set `frameId` to the frame's ID.
- **Dashed rectangles** - simpler visual grouping without clipping. Add standalone text label at top-left.
